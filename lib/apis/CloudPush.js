...
class CloudPush extends ApiCommon {
   ...
   *
   * @param {PushCommonArgs & PushTask} apiArgs
   */
  push(apiArgs) {
    apiArgs.Action = 'Push';
  ...
  /**
   * Mash push，具体参见[文档](https://help.aliyun.com/document_detail/159534.htm)
   * @param {PushCommonArgs} commonArgs
   * @param {PushTask[]} pushTasks
   */
  massPush(commonArgs, pushTasks) {
    if (_.isEmpty(pushTasks)) return [];

    const apiArgs = {
      Action: 'MassPush',
      SignatureNonce: _.get(commonArgs, 'SignatureNonce'),
      Timestamp: _.get(commonArgs, 'Timestamp'),
    };

    // prettier-ignore
    for (let i = 0; i < pushTasks.length; i++) {
      const task = pushTasks[i];
      const number = i + 1;

      apiArgs[`PushTask.${number}.DeviceType`] = 'ANDROID';
      apiArgs[`PushTask.${number}.AndroidPopupTitle`] = stringPreview(task.Title, 15);
      apiArgs[`PushTask.${number}.AndroidPopupBody`]  = stringPreview(task.Body, 127);
      apiArgs[`PushTask.${number}.Title`]             = task.Title;
      apiArgs[`PushTask.${number}.Body`]              = task.Body;
      apiArgs[`PushTask.${number}.Target`]            = _.get(task, 'Target', 'DEVICE');
      apiArgs[`PushTask.${number}.TargetValue`]       = _.get(task, 'TargetValue');
      apiArgs[`PushTask.${number}.PushType`]          = _.get(task, 'PushType', 'NOTICE');
      apiArgs[`PushTask.${number}.AndroidOpenType`]   = _.get(task, 'AndroidOpenType', 'APPLICATION');
      apiArgs[`PushTask.${number}.AndroidNotifyType`] = _.get(task, 'AndroidNotifyType', 'VIBRATE');
      apiArgs[`PushTask.${number}.AndroidActivity`]   = _.get(task, 'AndroidActivity');
      apiArgs[`PushTask.${number}.AndroidOpenUrl`]    = _.get(task, 'AndroidOpenUrl');
      apiArgs[`PushTask.${number}.AndroidNotificationBarType`]     = _.get(task, 'AndroidNotificationBarType', 50);
      apiArgs[`PushTask.${number}.AndroidNotificationBarPriority`] = _.get(task, 'AndroidNotificationBarPriority', 0);
      apiArgs[`PushTask.${number}.AndroidNotificationChannel`]     = _.get(task, 'AndroidNotificationChannel');
      apiArgs[`PushTask.${number}.AndroidExtParameters`]           = _.get(task, 'AndroidExtParameters');
      apiArgs[`PushTask.${number}.AndroidPopupActivity`]           = _.get(task, 'AndroidPopupActivity');
      apiArgs[`PushTask.${number}.AndroidRemind`] = _.get(task, 'AndroidRemind', true);
      apiArgs[`PushTask.${number}.StoreOffline`]  = _.get(task, 'StoreOffline', true);
    }
