...
const { generateToken, stringPreview, excludeNilKey, postForm } = require('./utils');
 ...
 * @param {{ extParams:object, deviceIds:string, androidActivity:string, androidPopupActivity:string, androidOpenType:AOpenType, timestamp:string, androidNotifyType:ANotiType, androidChannel:string, title:string, body:string, appKey:string, accessKeyId:string, accessKeySecret:string, format:'JSON'|'XML', nonce:string }} args
...
async function push(args) {
  ...
  const { androidActivity, androidPopupActivity, androidChannel, androidNotifyType, androidOpenType } = args;
  const query = {
    ...
    AndroidPopupActivity: androidPopupActivity, // String 否 此处指定通知点击后跳转的Activity。注：原AndroidXiaoMiActivity参数已废弃，所有第三方辅助弹窗都由新参数统一支持。
  ...
  return await postForm('https://cloudpush.aliyuncs.com', query);
}
